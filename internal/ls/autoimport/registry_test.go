...
func TestRegistryLifecycle(t *testing.T) {
	...
	t.Run("builds project and node_modules buckets", func(t *testing.T) {
		...
		defer release()
		_, err := session.GetLanguageServiceWithAutoImports(context.Background(), snapshot, mainFile.URI())
...
export const bar = 2;`,
		...
		snapshot, release := session.Snapshot()
		defer release()
		_, err := session.GetLanguageServiceWithAutoImports(ctx, snapshot, indexURI)
		assert.NilError(t, err)
		stats := autoImportStats(t, session)
		projectBucket := singleBucket(t, stats.ProjectBuckets)
	...
	t.Run("package.json dependency changes invalidate node_modules buckets", func(t *testing.T) {
		...
		snapshot, release := session.Snapshot()
		defer release()
		_, err := session.GetLanguageServiceWithAutoImports(ctx, snapshot, mainFile.URI())
		assert.NilError(t, err)
		stats := autoImportStats(t, session)
		nodeModulesBucket := singleBucket(t, stats.NodeModulesBuckets)
		assert.Equal(t, nodeModulesBucket.State.Dirty(), false)
	...
	t.Run("node_modules buckets get deleted when no open files can reference them", func(t *testing.T) {
		...
		snapshot, release := session.Snapshot()
		defer release()
		_, err := session.GetLanguageServiceWithAutoImports(ctx, snapshot, fileA.URI())
		assert.NilError(t, err)
		...
		snapshot, release = session.Snapshot()
		defer release()
		_, err = session.GetLanguageServiceWithAutoImports(ctx, snapshot, fileB.URI())
		assert.NilError(t, err)
		stats := autoImportStats(t, session)
	...
	t.Run("node_modules bucket dependency selection changes with open files", func(t *testing.T) {
		...
		snapshot, release := session.Snapshot()
		defer release()
		_, err := session.GetLanguageServiceWithAutoImports(ctx, snapshot, monorepoHandle.URI())
		assert.NilError(t, err)
		stats := autoImportStats(t, session)
		...
		snapshot, release = session.Snapshot()
		defer release()
		_, err = session.GetLanguageServiceWithAutoImports(ctx, snapshot, packageAHandle.URI())
		assert.NilError(t, err)
		stats = autoImportStats(t, session)
	...
	t.Run("node_modules bucket includes resolved packages from all projects", func(t *testing.T) {
		...
		snapshot, release = session.Snapshot()
		defer release()
		_, err = session.GetLanguageServiceWithAutoImports(ctx, snapshot, packageBHandle.URI())
		assert.NilError(t, err)
...
`,
		...
		snapshot, release := session.Snapshot()
		defer release()
		_, err := session.GetLanguageServiceWithAutoImports(ctx, snapshot, projectAURI)
		assert.NilError(t, err)
	...
	t.Run("changed fileExcludePatterns triggers bucket rebuild", func(t *testing.T) {
		...
		_, err = session.GetLanguageServiceWithAutoImports(ctx, snapshot2, mainFile.URI())
		assert.NilError(t, err)
		release2()
